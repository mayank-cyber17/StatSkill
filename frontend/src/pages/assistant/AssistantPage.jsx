    ...
    <div className="space-y-3 leading-relaxed text-sm">
      {parts.map((part, index) => {
        if (part.startsWith('```')) {
          ...
          return (
            <div key={index} className="rounded-xl overflow-hidden border border-white/10 bg-surface-950/80 my-2 font-mono text-xs shadow-lg">
              <div className="flex items-center justify-between px-3 py-1.5 bg-surface-900 border-b border-white/10 text-slate-400">
          ...
          <div key={index} className="space-y-1.5">
            {lines.map((line, lineIdx) => {
              ...
              if (trimmed.startsWith('### ')) {
                return (
                  <h4 key={lineIdx} className="text-base font-bold text-white pt-2 pb-0.5 text-brand-300">
              ...
              if (trimmed.startsWith('## ')) {
                return (
                  <h3 key={lineIdx} className="text-lg font-bold text-white pt-2 pb-1 text-accent-300">
                    {parseInline(trimmed.replace(/^##\s+/, ''))}
                  </h3>
                )
              }
              ...
              if (numMatch) {
                return (
                  <div key={lineIdx} className="flex items-start gap-2 pl-2">
                    <span className="font-semibold text-brand-400 text-xs mt-0.5 min-w-[1.2rem]">
                      {numMatch[1]}.
                    </span>
                    <span className="flex-1 text-slate-200">
...
function parseInline(text) {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return (
        <code key={i} className="px-1.5 py-0.5 rounded bg-surface-900 border border-white/10 text-brand-300 font-mono text-[12px]">
          {part.slice(1, -1)}
        </code>
      )
    }
...
export default function AssistantPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto h-[calc(100vh-130px)] flex gap-4">
      {/* Sessions Sidebar */}
      <div className="hidden md:flex flex-col w-64 bg-surface-800/70 border border-white/10 rounded-2xl p-3 shrink-0 backdrop-blur-md">
        <button
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
          {conversations.length === 0 ? (
            <div className="text-xs text-slate-500 text-center py-6 px-2">
            ...
            conversations.map((c) => (
              ...
              >
                <div className="flex items-center gap-2 truncate flex-1 min-w-0">
                  <MessageSquare className="w-3.5 h-3.5 shrink-0 text-brand-300" />
        ...
        {/* Status Pill */}
        <div className="pt-3 border-t border-white/10 mt-auto flex items-center gap-2 px-2 text-[11px] text-slate-400">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
          <span className="text-slate-300 font-medium">InnoWing Engine Ready</span>
        </div>
      </div>

      {/* Main Chat Interface */}
      <div className="flex-1 flex flex-col bg-surface-800/60 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-surface-800/90">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-display font-bold text-white tracking-tight">
                  InnoWing <span className="text-brand-400">AI</span>
                </h1>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleNewChat}
              className="md:hidden btn btn-ghost text-xs border border-white/10"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
          {messages.map((m, idx) => (
            ...
            >
              ...
              <div
                className={`max-w-[85%] rounded-2xl p-4 transition-all shadow-md ${
                  m.role === 'USER'
                    ...
                    : 'bg-surface-900/90 border border-white/10 text-slate-100 rounded-bl-none'
                }`}
              >
                {m.role === 'ASSISTANT' ? (
                  <div>
                    <FormattedContent content={m.content} />
                    <div className="flex items-center justify-between pt-3 mt-3 border-t border-white/10 text-slate-400 text-[11px]">
                      <span className="flex items-center gap-1 text-slate-500">
                        <Sparkles className="w-3 h-3 text-brand-400" /> InnoWing Intelligence
                      </span>
                      <button
                        onClick={() => handleCopyMessage(m.content)}
                        className="hover:text-white transition-colors flex items-center gap-1 px-2 py-0.5 rounded bg-white/5 hover:bg-white/10"
                        title="Copy text"
                      >
                        <Copy className="w-3 h-3" /> Copy
                      </button>
              ...
              {m.role === 'USER' && (
                <div className="w-8 h-8 rounded-xl bg-surface-700 flex items-center justify-center text-brand-300 shrink-0 border border-white/10 mt-0.5">
          ...
          {loading && (
            <div className="flex gap-3 justify-start items-start">
              ...
              <div className="bg-surface-900/90 border border-white/10 rounded-2xl rounded-bl-none p-4 text-xs text-slate-300 flex items-center gap-2.5">
        ...
        <div className="px-6 py-2 border-t border-white/10 bg-surface-850/40 flex items-center gap-2 overflow-x-auto custom-scrollbar">
          <span className="text-[11px] text-slate-400 font-semibold uppercase shrink-0 flex items-center gap-1">
          ...
          {quickPrompts.map((qp, idx) => (
            <button
              ...
              className="text-xs shrink-0 bg-surface-700/60 hover:bg-brand-600/30 text-slate-300 hover:text-white px-3 py-1 rounded-full border border-white/10 transition-all flex items-center gap-1.5"
            >
              {qp.label}
            </button>
          ))}
        ...
        <form
          ...
          className="p-4 border-t border-white/10 bg-surface-850 flex gap-2 items-center"
        >
          <input
            type="text"
            value={input}
            ...
            className="input flex-1 bg-surface-900 border-white/10 text-white placeholder:text-slate-500 focus:border-brand-500 text-sm py-3 px-4 rounded-xl"
            disabled={loading}
          />
          <button
            type="submit"
